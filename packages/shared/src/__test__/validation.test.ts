...
describe('validation schemas', () => {
    describe('SharedSecretSchema', () => {
        ...
        it('should reject arrays with wrong length', () => {
            ...
            expect(result.success).toBe(false);
            expect(result.error?.issues[0]?.message).toContain(
                'SharedSecret must be exactly 32 numbers',
            );
        });
    ...
    describe('Request schemas', () => {
        ...
        it('should validate valid import account request', () => {
            ...
            expect(
                ImportAccountRequestSchema.safeParse(validData).success,
            ).toBe(true);
        });

        it('should reject invalid requests', () => {
            const invalidData = { status: 'invalid' };
            expect(PairRequestSchema.safeParse(invalidData).success).toBe(
                false,
            );
        });
    });
    ...
    describe('StoredRequestSchema', () => {
        ...
        it('should reject invalid stored request', () => {
            ...
            expect(StoredRequestSchema.safeParse(invalidData).success).toBe(
                false,
            );
        });
    });
