...
function BalanceChangeItem({ delta, tokenInfo }: BalanceChangeItemProps) {
    ...
    return (
        <List.Item
            title={singleLineDisplay}
        />
    );
}
export function TxSimulation({ network, from, transaction }: TxSimulationProps) {
    ...
    const runSimulation = async () => {
        ...
        try {
            ...
            const result = await client.simulate(network, {
                ...
                gas: transaction.gas ? `0x${transaction.gas.toString(16)}` : "0x1E8480",
                block_number: "latest",
            });
                    ...
                    <>
                        {simulation.balance_deltas.length > 0 ? (
                            <View>
                                ...
                                {simulation.balance_deltas.map((delta, index) => (
                                    <View key={`${delta.address}-${delta.token}-${index}`}>
                                        <BalanceChangeItem
                                            delta={delta}
                                            tokenInfo={simulation.tokens[delta.token.toLowerCase()]}
                                        />
                                        {index < simulation.balance_deltas.length - 1 && <Divider />}
                                    </View>
                                ))}
                            </View>
                        ) : (
                            <Text variant="bodyMedium">No balance changes detected</Text>
                        )}
                    </>
                )}
            </Card.Content>
